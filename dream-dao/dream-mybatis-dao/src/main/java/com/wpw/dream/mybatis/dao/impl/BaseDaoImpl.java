package com.wpw.dream.mybatis.dao.impl;
...
import com.wpw.dream.mybatis.dao.BaseDao;
import com.wpw.dream.mybatis.dao.entity.BaseEntity;
import com.wpw.dream.mybatis.dao.exception.BizException;
import com.wpw.dream.mybatis.dao.page.PageBean;
import com.wpw.dream.mybatis.dao.page.PageParam;
...
public abstract class BaseDaoImpl<T extends BaseEntity> extends SqlSessionDaoSupport implements BaseDao<T> {
	...
	public int update(T entity) {
		int result = getSqlSession().update(getStatement(SQL_UPDATE), entity);
		if (result <= 0) {
	...
	public int update(List<T> list) {
		...
		int result = getSqlSession().update(getStatement(SQL_BATCH_UPDATE), list);
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,update返回0.{%s}", getStatement(SQL_UPDATE));
		}
		return result;
	...
	public int deleteById(long id) {
		return (int) getSqlSession().delete(getStatement(SQL_DELETE_BY_ID), id);
	}
	...
	public PageBean listPage(PageParam pageParam, Map<String, Object> paramMap) {
		...
		// 统计总记录数
		Long count = getSqlSession().selectOne(getStatement(SQL_LIST_PAGE_COUNT), paramMap);
	...
	public List<T> listBy(Map<String, Object> paramMap) {
		return getSqlSession().selectList(getStatement(SQL_LIST_BY), paramMap);
	}
	...
	public T getBy(Map<String, Object> paramMap) {
		...
		return getSqlSession().selectOne(getStatement(SQL_LIST_BY), paramMap);
	}
